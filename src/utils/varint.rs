...
pub fn write_varint_unsigned<W: Write>(i: u32, buffer: &mut W) -> Result<(), std::io::Error> {
...
pub fn read_varint_unsigned<'a>(i: &mut u32, buffer: &'a [u8]) -> &'a [u8] {
    let rem = &buffer[1..];
    match buffer[0] {
        253 => {
...
/// to not use too much space
pub fn write_varint_signed<W: Write>(i: i64, buffer: &mut W) -> Result<(), std::io::Error> {
    ...
    if i >= 0 {
        if i <= 148 {
            buffer.write_all(&[i as u8 + 103])
        } else if i <= u8::MAX as i64 {
            buffer.write_all(&[252, i as u8])
        } else if i <= u16::MAX as i64 {
            buffer.write_all(&[253])?;
            buffer.write_all((i as u16).to_be_bytes().as_ref())
        } else if i <= u32::MAX as i64 {
            buffer.write_all(&[254])?;
            buffer.write_all((i as u32).to_be_bytes().as_ref())
        } else {
            buffer.write_all(&[255])?;
            buffer.write_all(i.to_be_bytes().as_ref())
        }
    } else {
        if i >= -99 {
            buffer.write_all(&[(i + 103) as u8])
        } else if i >= -(u8::MAX as i64) {
            buffer.write_all(&[3, !(-i as u8)])
        } else if i >= -(u16::MAX as i64) {
            buffer.write_all(&[2])?;
            buffer.write_all((!(-i as u16)).to_be_bytes().as_ref())
        } else if i >= -(u32::MAX as i64) {
...
pub fn read_varint_signed<'a>(i: &mut i64, buffer: &'a [u8]) -> &'a [u8] {
...
mod tests {
    ...
    fn test_varint_signed() {
        ...
        for i in &numbers {
            let mut buf = vec![];
            write_varint_signed(*i, &mut buf).unwrap();
            asc_byte_arrays.push(buf);
        }
